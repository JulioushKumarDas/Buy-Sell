<!doctype html>
<html lang="en">
  <head>
    <!-- Required meta tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">

    <!-- Bootstrap CSS -->
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css" integrity="sha384-ggOyR0iXCbMQv3Xipma34MD+dH/1fQ784/j6cY/iJTQUOhcWr7x9JvoRxT2MZw1T" crossorigin="anonymous">

    <title>Stutus of product</title>
	 <style>
        .new1{height:100px;
                text-align:center;
                background-color:navy;
                color:white;}
            .nav-link
                {
                        color :navy;
                }
                .btn
                {
                        background-color:navy;
                }
                .btn:hover
		{
                        background-color:red;
                        width:140px;
                }
		 .jumbotron
                {
                        background-color:navy;
                        text-align:center;
                        color:white;
                }
                .jumbotron:hover
                {
                        background-color:red;
                }

		.container1{
			background-color:rgba(225,55,55,0.5);
			height:40px;
			border:1px solid navy;
			}
		.row1{background-color:rgba(125,55,55,0.5);
			height:25px;}
	</style>
  </head>
  <body>
	<body>
   <!-- A grey horizontal navbar that becomes vertical on small screens -->
        <nav class="navbar navbar-expand-sm bg-light sticky-top">
          <a class="navbar-brand" href="#">
                <img src="logo2.png" alt="Logo" style="width:40px;height:40px;">
          </a>
          <!-- Links -->
          <ul class="navbar-nav ">
            <li class="nav-item ">
              <a class="nav-link " href="homepageecomm.php">Home</a>
            </li>
            <li class="nav-item">
              <a class="nav-link " href="sell.php">Sell</a>
            </li>
            <li class="nav-item">
              <a class="nav-link " href="loginpage.php">SignUp/Login</a>
            </li>
                <li class="nav-item dropdown">
                <a class="nav-link dropdown-toggle" href="#" id="navbardrop" data-toggle="dropdown">Category</a>
              <div class="dropdown-menu">
                <a class="dropdown-item " href="noveltry1.php">Novel</a>
                <a class="dropdown-item" href="syllabus1.php">Syllabus Books</a>
                <a class="dropdown-item" href="other1.php">Others</a>
              </div>
            </li>
          </ul>
	  <form class="form-inline " action="/action_page.php">
            <input class="form-control mr-sm-2 " type="text" placeholder="Search">
            <button class="btn btn-outline-success my-2 my-sm-0" type="submit">Search</button>
          </form>
        </nav>
        <div class="new1">
                <h1>Your selected Product Status</h1>
        </div></br>

	<div class="container container1">
	  <div class="row">
	    <div class="col"><b>Book Title</b></div>
	    <div class="col"><b>Customer Name</b></div>
	    <div class="col"><b>Customer EmailId</b></div>
	    <div class="col"><b>Phone no</b></div>
	</div></br>
	</div>
	<div class="container">
	<?php
        $con=mysqli_connect('localhost','root',);
        mysqli_select_db($con,'userregistration');
        $title=$_GET['tn2'];
        $q="select * from ordertable where title='$title'";
        $result=$con->query($q);
        $n=mysqli_num_rows($result);
        if($n>0)
        {
                while($row=$result->fetch_assoc())
                {
                        $name=$row['username'];
                        $email=$row['email'];
			$phone=$row['contactno'];
                        $title=$row['title'];
	?>		<div class="row row1">
            			<div class="col"><?php echo ($title);?></div>
            			<div class="col"><?php echo ($name);?></div>
            			<div class="col"><?php echo ($email);?></div>
            			<div class="col"><?php echo ($phone);?></div>
        		</div></br>
		<?php
		}
	}
	?>
	</div>
</br>	</br>
<div class="jumbotron">
  <h1 class="display-4">About Us</h1>
  <p class="lead">Please, Visit Again</p>
  <hr class="my-4">
  <p>For Help Go to Learn More</p>
  <a class="btn btn-primary btn-lg" href="learn.php" role="button">Learn more</a>
</div>

    <!-- Optional JavaScript -->
    <!-- jQuery first, then Popper.js, then Bootstrap JS -->
    <script src="https://code.jquery.com/jquery-3.3.1.slim.min.js" integrity="sha384-q8i/X+965DzO0rT7abK41JStQIAqVgRVzpbzo5smXKp4YfRvH+8abtTE1Pi6jizo" crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js" integrity="sha384-UO2eT0CpHqdSJQ6hJty5KVphtPhzWj9WO1clHTMGa3JDZwrnQq4sF86dIHNDz0W1" crossorigin="anonymous"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js" integrity="sha384-JjSmVgyd0p3pXB1rRibZUAYoIIy6OrQ6VrjIEaFf/nJGzIxFDsf4x0xIM+B07jRM" crossorigin="anonymous"></script>
  </body>
</html>
